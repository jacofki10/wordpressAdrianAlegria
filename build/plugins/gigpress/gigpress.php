...
function gigpress_prepare($show, $scope = 'public')
{
	...
	if ($scope != 'venue') {
		...
		$showdata['date_long2'] = mysql2date($gpo['date_format_long2'], $show->show_date);
		...
		switch ($showdata['status']) {
			case 'active':
				$showdata['ticket_link'] = ($show->show_tix_url && $show->show_expire >= GIGPRESS_NOW) ? '<a href="' . esc_url($show->show_tix_url)  . '"' . gigpress_target($show->show_tix_url) . ' class="gigpress-tickets-link" target="_blank">' . wptexturize($gpo['buy_tickets_label']) . '</a>' : '';
				break;
			case 'soldout':
		...
		if ($showdata['related_url']) {
			...
			$showdata['permalink'] = esc_url($gpo['shows_page']);
		} else {
			$showdata['permalink'] = get_bloginfo('url');
		}
