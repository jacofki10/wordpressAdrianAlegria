...
function gigpress_prepare_show_fields($context = 'new')
{
	...
	if ($_POST['show_related'] == "new") {
		...
		$replace = array(
			...
			mysql2date($gpo['date_format_long2'], $show['show_date']),
	...
	if ($errors) {
		...
		<?php } elseif ($addshow === FALSE) { ?>
			...
			<div id="message" class="error fade">
				<p><?php _e("Something ain't right - try again?", "gigpress"); ?></p>
			</div>
...
function gigpress_update_show()
{
	...
	if ($errors) {
		...
		if ($updateshow != FALSE) {
			...
			?>
			<div id="message" class="updated fade">
				<p><?php echo __("Your show  on", "gigpress") . ' ' . mysql2date($gpo['date_format_long'], $show['show_date']) . ' ' . __("was successfully updated.", "gigpress");
						if ($show['show_related']) echo (' <a href="' . admin_url('post.php?action=edit&amp;post=' . $show['show_related']) . '">' . __("Edit the related post", "gigpress") . '.</a>'); ?></p>
				<?php
				global $errors;
				if ($errors) {
					foreach ($errors as $error) {
...
function gigpress_add_venue()
{
	...
	if ($errors) {
		...
		if ($addvenue != FALSE) { ?>
			<div id="message" class="updated fade">
				<p><?php echo wptexturize($venue['venue_name']) . ' ' . __("was successfully added to the database.", "gigpress"); ?></p>
			</div>
...
function gigpress_update_venue()
{
	...
	if ($errors) {
		...
		if ($updatevenue != FALSE) { ?>
			<div id="message" class="updated fade">
				<p><?php echo wptexturize($venue['venue_name']) . ' ' . __("was successfully updated.", "gigpress"); ?></p>
			</div>
...
function gigpress_delete_venue()
{
	...
	if ($trashvenue != FALSE) {	?>
		<div id="message" class="updated fade">
			<p><?php _e("Venue successfully deleted.", "gigpress"); ?></p>
		</div>
	<?php } elseif ($trashvenue === FALSE) { ?>
		<div id="message" class="error fade">
			<p><?php _e("We ran into some trouble deleting the venue. Sorry.", "gigpress"); ?></p>
		</div>
	<?php }
}
...
function gigpress_add_tour()
{
	...
	if ($errors) {
		...
		if ($addtour != FALSE) { ?>
			<div id="message" class="updated fade">
				<p><?php echo wptexturize($tour['tour_name']) . ' ' . __("was successfully added to the database.", "gigpress"); ?></p>
			</div>
...
function gigpress_update_tour()
{
	...
	if ($errors) {
		...
		if ($updatetour != FALSE) { ?>
			<div id="message" class="updated fade">
				<p><?php _e("Tour name successfully changed to", "gigpress");
						echo ': ' . wptexturize($tour['tour_name']); ?></p>
			</div>
...
function gigpress_delete_tour()
{
	...
	if ($trashtour != FALSE) {
		...
		<div id="message" class="updated fade">
			<p><?php _e("Tour successfully deleted.", "gigpress"); ?> <small>(<a href="<?php echo $undo; ?>"><?php _e("Undo", "gigpress"); ?></a>)</small></p>
		</div>

	<?php } elseif ($trashtour === FALSE) { ?>

		<div id="message" class="error fade">
			<p><?php _e("We ran into some trouble deleting the tour. Sorry.", "gigpress"); ?></p>
		</div>
	<?php }
}
...
function gigpress_add_artist()
{
	...
	if ($errors) {
		...
		if ($addartist != FALSE) { ?>
			<div id="message" class="updated fade">
				<p><?php echo wptexturize($artist['artist_name']) . ' ' . __("was successfully added to the database.", "gigpress"); ?></p>
			</div>
...
function gigpress_update_artist()
{
	...
	if ($errors) {
		...
		if ($updateartist != FALSE) { ?>
			<div id="message" class="updated fade">
				<p><?php echo wptexturize($artist['artist_name']) . ' ' . __("successfully updated.", "gigpress"); ?></p>
			</div>
...
function gigpress_delete_artist()
{
	...
	if ($trashartist != FALSE) {	?>
		<div id="message" class="updated fade">
			<p><?php _e("Artist successfully deleted.", "gigpress"); ?></p>
		</div>
	<?php } elseif ($trashartist === FALSE) { ?>

		<div id="message" class="error fade">
			<p><?php _e("We ran into some trouble deleting the artist. Sorry.", "gigpress"); ?></p>
		</div>
	<?php }
}
...
if ($type == "tour") {
	...
	if ($undo != FALSE) { ?>
			<div id="message" class="updated fade">
				<p><?php _e("Tour successfully restored from the database.", "gigpress"); ?></p>
			</div>
		<?php } elseif ($undo === FALSE) { ?>
			<div id="message" class="error fade">
				<p><?php _e("We ran into some trouble restoring the tour. Sorry.", "gigpress"); ?></p>
			</div>
		<?php }
}
}
...
function gigpress_empty_trash()
{
	...
	if ($trashshows || $trashtours) { ?>
		<div id="message" class="updated fade">
			<p><?php _e("All shows and tours in the trash have been permanently deleted.", "gigpress"); ?></p>
		</div>
	<?php } else { ?>
		<div id="message" class="error fade">
			<p><?php _e("We ran into some trouble emptying the trash. Sorry.", "gigpress"); ?></p>
		</div>
	<?php }
}
